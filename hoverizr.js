...
$.fn.hoverizr = function(options) {
	...
	if($('.post-thumb-holder img').css('maxWidth') != "none"){
		var maxwidth = 1;
		$('.post-thumb-holder img').css('maxWidth',"none");
	} else{
		var maxwidth = 0;
	}
	...
	this.each(function(){
		...
		default:
			...
			for (var i = 0, n = pix.length; i < n; i += 4) {
				var grayscale = pix[i  ] * .1 + pix[i+1] * .64 + pix[i+2] * .13; //Very luminus grayscale formula
				//var grayscale = (pix[i  ] + pix[i+1] + pix[i+2])/3   - average grayscale formula
				//var grayscale = (pix[i  ] + pix[i+1] + pix[i+2])/3   - 
				pix[i  ] = grayscale +10; 	// red
				pix[i+1] = grayscale +6; 	// green
				pix[i+2] = grayscale +5; 	// blue
				// i+3 is alpha (the fourth element)
			}
			break;
		}
	...
	if(maxwidth == 1){
		$('.post-thumb-holder img').css('maxWidth', "100%");
	}
